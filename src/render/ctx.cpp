...
void t_camera::apply ()
{
	...
	render_ctx.proj = proj = perspective(radians(fov), aspect, z_near, z_far);
	render_ctx.view = view = rotate_xyz(radians(ang - vec3(90.0, 0.0, 0.0)));
	render_ctx.view = view = translate(render_ctx.view, -pos);
