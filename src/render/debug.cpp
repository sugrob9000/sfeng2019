...
void init_render_debug() {
  prog_tex2d =
    make_glsl_program({get_vert_shader("internal/debug/screen_quad"), get_frag_shader("internal/debug/tex2d")}
    );
  glUseProgram(prog_tex2d);
  glUniform1i(UNIFORM_LOC_TEX, 0);

  prog_tex2d_array = make_glsl_program(
    {get_vert_shader("internal/debug/screen_quad"), get_frag_shader("internal/debug/tex2d_array")}
  );
...
void debug_render_tex2d_array(GLuint tex, int layer, float x, float y, float size) {
  glDisable(GL_DEPTH_TEST);
  glDisable(GL_CULL_FACE);
  glUseProgram(prog_tex2d_array);
