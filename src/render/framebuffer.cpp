...
Framebuffer& Framebuffer::assert_complete() {
  ...
  if (width <= 0 || height <= 0) {
    const char* hint = (width == 0 && height == 0) ? " - perhaps nothing has been attached?" : "";
...
static void attach_low(Framebuffer& fbo, Framebuffer::AttachmentRef att, GLenum slot, int slice) {
  if (fbo.width == 0 && fbo.height == 0) {
    ...
    fatal(
      "Tried to attach object %i with dimensions %i, %i "
  ...
  switch (att->target) {
  case tex2d:
    glFramebufferTexture2D(GL_FRAMEBUFFER, slot, GL_TEXTURE_2D, att->id, 0);
    break;
  case tex2d_msaa:
    glFramebufferTexture2D(GL_FRAMEBUFFER, slot, GL_TEXTURE_2D_MULTISAMPLE, att->id, 0);
    break;
  case tex2d_array:
    glFramebufferTextureLayer(GL_FRAMEBUFFER, slot, att->id, 0, slice);
    break;
  case tex2d_array_msaa:
    glFramebufferTexture3D(GL_FRAMEBUFFER, slot, GL_TEXTURE_2D_MULTISAMPLE_ARRAY, att->id, 0, slice);
    break;
  case rbo:
  case rbo_msaa:
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, slot, GL_RENDERBUFFER, att->id);
    break;
  default:
    fatal("FBO %i, attachment id %i: invalid target enum %i", fbo.id, att->id, att->target);
  }
}
...
Framebuffer& Framebuffer::attach_color(FramebufferAttachment* att, int idx, short slice) {
  ...
  if (color[idx].taken()) {
    warning(
      "Replacing existing color attachment #%i on FBO %i "
...
inline FramebufferAttachment* attachment_finalize(FramebufferAttachment* a) {
  ...
  case tex2d:
    ...
    glTextureStorage2D(a->id, 1, a->storage_type, a->width, a->height);
    break;
  case tex2d_msaa:
    t = GL_TEXTURE_2D_MULTISAMPLE;
    glGenTextures(1, &a->id);
    glBindTexture(t, a->id);
    glTextureStorage2DMultisample(a->id, a->samples, a->storage_type, a->width, a->height, true);
    break;
  case tex2d_array:
    ...
    glTexParameteri(t, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(t, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    glTextureStorage3D(a->id, 1, a->storage_type, a->width, a->height, a->depth);
    break;
  case tex2d_array_msaa:
    glGenTextures(1, &a->id);
    glBindTexture(GL_TEXTURE_2D_MULTISAMPLE_ARRAY, a->id);
    glTextureStorage3DMultisample(a->id, a->samples, a->storage_type, a->width, a->height, a->depth, true);
    break;
  case rbo:
    glGenRenderbuffers(1, &a->id);
    glBindRenderbuffer(GL_RENDERBUFFER, a->id);
    glRenderbufferStorage(GL_RENDERBUFFER, a->storage_type, a->width, a->height);
    break;
  case rbo_msaa:
    glGenRenderbuffers(1, &a->id);
    glBindRenderbuffer(GL_RENDERBUFFER, a->id);
    glRenderbufferStorageMultisample(GL_RENDERBUFFER, a->samples, a->storage_type, a->width, a->height);
    break;
  }
