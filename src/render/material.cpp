...
void t_material::load(const std::string& path) {
  ...
  for (const auto& d: bitmaps) {
    int location = glGetUniformLocation(program, d.loc_name.c_str());

    if (location == -1) {
      warning("%s is not a valid uniform in material %s", d.loc_name.c_str(), path.c_str());
      continue;
    }
...
void t_material::apply() const {
  if (!can_skip_application(this)) {
    ...
    for (int i = 0; i < bitmap_texture_ids.size(); i++) {
      bind_tex2d_to_slot(MAT_TEXTURE_SLOT_OFFSET + i, bitmap_texture_ids[i]);
    }
  }
...
GLenum get_surface_gl_format(SDL_Surface* s) {
  ...
  uint16_t i = compress(s->format->Amask) | (compress(s->format->Rmask) << 4)
    | (compress(s->format->Gmask) << 8) | (compress(s->format->Bmask) << 12);
  switch (i) {
...
static bool append_glsl_source(
  ...
  if (depth > maxdepth) {
    warning(
      "Shader %s: depth of #include chain exceeds %i. "
