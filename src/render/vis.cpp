...
void init_vis() {
  occ_planes_prog =
    make_glsl_program({get_vert_shader("internal/vis_plane"), get_frag_shader("common/null")});

  occ_cube_prog = make_glsl_program({get_vert_shader("internal/vis_cuboid"), get_frag_shader("common/null")});
...
void t_visible_set::fill() {
  ...
  while (!queues[cur_queue].empty()) {
    ...
    for (oct_node* n: queues[cur_queue]) {
      ...
      for (int i = 0; i < 8; i++) {
        glBeginQuery(GL_SAMPLES_PASSED, n->children[i].query);

        glUniform3fv(UNIFORM_LOC_VIS_CUBE, 2, n->children[i].bounds.data());
...
void read_world_vis_data(std::string path) {
  ...
  while (f >> option) {
    if (option == "oct_depth") {
      ...
      f >> world_bounds_override.start >> world_bounds_override.end;
    } else {
      warning("Unrecognized option in vis data %s: %s", path.c_str(), option.c_str());
    }
  }
}
